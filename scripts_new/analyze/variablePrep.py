...
df = pd.read_csv("other_source/final_tota.csv")

df = df[df.topic_no != -1] # drop the others categories

df['clause_id'] = df.apply(lambda x: x['chapter_cat'] + "_" + x['topic'],axis=1)
...
tradedata = pd.read_csv('other_source/tradeall.csv')
...
exportdf.to_csv('other_source/analyze_exportdf.csv', index=False)