...
df = pd.read_csv("core_excels/totaPlus_afterManualReclass.csv")
...
df['cumulative'] = df.apply(lambda x: int(1) if ( (x['year'] >= x['dif'] ) & (x['year'] <= x['din']) ) else int(0), axis=1)

#df['delta'] = df.apply(lambda x: int(1) if ( (x['year'] == x['dif'] )) else int(0), axis=1)
...
df = df.groupby(['parties_perm', 'year', 'clause_id']).sum(['cumulative'])
...
df=df.pivot(index=['year', 'parties_perm'], columns = 'clause_id', values=['cumulative'] )

df.columns = [tuple[1] +"_"+ tuple[0] for tuple in  df.columns.to_flat_index()]
...
exportdf = pd.concat([finaldf[['export_value', 'exporter','importer', 'parties_perm']], integered_col],axis=1)
...
# export and use R :D
exportdf = exportdf.drop(['Unnamed: 0'],axis=1)
exportdf.to_csv('analyze_exportdf.csv', index=False)
...
yes = pd.read_excel('ftamodels.xlsx',0)
yes = yes.merge(pd.read_excel('ftamodels.xlsx',1),how='outer').merge(pd.read_excel('ftamodels.xlsx',2), how='outer').merge(pd.read_excel('ftamodels.xlsx',3), how='outer')

yes.to_csv('ftamodels.csv')
