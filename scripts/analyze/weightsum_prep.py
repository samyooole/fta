...
df=df[['parties', 'dif', 'din', 'clause_id', 'isMandatory_prob']]
...
df['cumulative'] = df.apply(lambda x: int(1) * x['isMandatory_prob'] if ( (x['year'] >= x['dif'] ) & (x['year'] <= x['din']) ) else int(0), axis=1)

df['delta'] = df.apply(lambda x: int(1) * x['isMandatory_prob'] if ( (x['year'] == x['dif'] )) else int(0), axis=1)
...
df = df.groupby(['parties_perm', 'year', 'clause_id']).sum(['cumulative','delta'])
...
df=df.pivot(index=['year', 'parties_perm'], columns = 'clause_id', values=['cumulative', 'delta'] )
...
finaldf.to_csv('analyze_exportdf.csv')
