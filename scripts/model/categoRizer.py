...
semcat = cleanandTransform(filters = [],  transformer_package='all-MiniLM-L12-v2')
semcat.set_filters(['to_lower', 'remove_url','remove_special_character', 'normalize_unicode', 'remove_whitespace'])
...
for chap in chaps:
    ...
    aggcat.init_text([string.replace("-", " ") for string in arts])
    ...
    """
    now, within the article categorizations, we use sentence transformers to cluster by semantic meaning
    """
    
    for art in pd.unique(newsmalldf.finalnewart):
        smallerdf = newsmalldf[newsmalldf['finalnewart'] == art]
        text=smallerdf.text
        semcat.init_text(text)
        semcat.process_text()
        semcat.transform_text()

        clausemeaninglabels = semclusterModel.fit_predict(semcat.current_embedding)
...
smallerdfpool.to_csv('core_excels/totaPlus_refinebranch.csv')
